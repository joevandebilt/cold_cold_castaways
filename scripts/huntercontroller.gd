...
func _ready() -> void: 
	...
	collect_craft.connect(craft_coat_reward)
...
func _process(delta: float) -> void:	
	if !awake and fire.fuel_available > 0:
		awake = true
		npc_dialogue.text = "I'm hunter, I can gather food or I can make coats to help us survive the cold, let me teach you how to gather too!\r\n\r\n*You have learnd how to gather food from bushes*"
		
	can_craft = has_craft_requirement()
	
	_base_process(delta)
	
func gather_food_reward():
	print("Finished Gathering Food")
	player.inventory.add_resource(player.inventory.Resources.FOOD, 200)
	
func craft_coat_reward():
